---
title: SQLite
description: Val Town SQLite is a lightweight, standard database. Every Val Town
  account comes with its own private SQLite database that is accessible from any
  of your vals.
sidebar:
  order: 0
lastUpdated: 2024-03-18
# cspell:ignore orms
---

[SQLite](https://www.sqlite.org/) is a lightweight, standard database. Every Val Town account comes with its own private SQLite database that is accessible from any of your vals via [`std/sqlite`](https://www.val.town/v/std/sqlite).

Val Town SQLite is powered by [Turso](https://turso.tech/).

- [Usage](./usage)
- [Migrations](./migrations)
- [ORMs](./orms)

You may find these admin viewers helpful managing your database if you need to drop tables or want to inspect your data:

- [LibSQL Studio](https://libsqlstudio.com/) (recommended - see instructions [here](https://libsqlstudio.com/docs/connect-valtown))
- [SQLite Explorer](https://www.val.town/v/nbbaier/sqliteExplorerApp) (built in Val Town)

## Limits

You can store 10mb on the free plan and up to 1gb on the paid plan. Contact us if you need more space.
