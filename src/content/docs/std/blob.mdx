...
Val Town now comes with [blob storage](https://www.cloudflare.com/en-gb/learning/cloud/what-is-blob-storage/) built-in. You can access it with [`std/blob` val](https://www.val.town/v/std/blob). It allows for storing any data: text, JSON, images. Blob storage is scoped globally to your account. If you set a blob in one val, you can retrieve it by the same key in another val. It's backed by Cloudflare R2.
...
Blob storage is scoped globally to your account. If you set a blob in one val, you can retrieve it by the same key in another val. It's backed by Cloudflare R2. Blob-stored data counts towards your total Val Town storage - 10mb on the free plan and 1gb on pro. Check our [pricing page](https://val.town/pricing) to learn more.
