...
This is a step-by-step guide to creating a cron job that sends you a weather email every morning.
...
Create a new val or use an existing one. Add a new TypeScript file (e.g., `weatherNotifier.ts`) to your val. In the file editor, set its trigger type to **Cron** via the UI.
...
:::tip
You'll notice that we're importing code from another val into this file. This is a common pattern in Val Town. If you type `@` in the editor, you'll be able to search and import other files inline.
:::
...
3. Click **Cron** (if not already selected) or edit the existing schedule.
4. Paste in your cron expression.

### Next steps

🥳 Congratulations! You've created a Cron job that sends you a weather notification every morning. You can customize it to your needs. There are a LOT of weather-related vals you can use or gain inspiration from:
