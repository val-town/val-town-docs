...
In the following example code, we are use `getWeather` from [stevekrouse/getWeather](https://val.town/v/stevekrouse/getWeather). It uses the free [wttr.in](https://wttr.in/:help) service to get weather data.
...
🥳 Congratulations! You've created a val that sends you a weather notification every morning. You can to customize it to your needs. There are a LOT of weather vals you can use or gain inspiration from:
